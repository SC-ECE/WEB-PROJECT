...
function Mage(scale) {
  ...
  this.jump = function (sy, sx) {
    ...
    x = Math.sqrt((map.tile_size * (map.tile_size * x + this.size / 2)) / 32);
    this.vel.y = -y;
      ...
      for (var x in this.data.background[y]) {
        ...
        for (var ty in tile) {
          ...
          for (var tx in tile[ty]) {
            ctx.fillStyle = this.data.colors[tile[ty][tx]];
            ctx.fillRect(
              x * this.tile_size + tx * width + this.offset.x,
              y * this.tile_size + ty * height + this.offset.y,
              width,
              height
            );
          }
...
function Minimap(map, colors) {
  ...
  var stop = function (e) {
    ...
    if (map_zoom && e.target === canvas) {
      ...
      (start_offset.x = self.offset.x), (start_offset.y = self.offset.y);
    }
  };
  ...
  var detailed_draw = function () {
    for (var y in self.map.data.background) {
      for (var x in self.map.data.background[y]) {
        ...
        ctx.fillRect(
          x * self.map.tile_size,
          y * self.map.tile_size,
          self.map.tile_size,
          self.map.tile_size
        );
      }
  ...
  this.draw = function () {
    if (canvas.parentNode === document.body) draw();
  };
}
...
function Joystick(callback) {
  var left_div, right_div, jump_div, shoot_div, change_div, pos, divs;
  ...
  var setup = function () {
    ...
    var change_func = function (e) {
      if (is_targeted(e.target, change_div)) {
        ...
        if (color === "rgb(0, 0, 0)") {
          ...
          for (var i in divs) divs[i].style.background = "red";
        } else {
          resume();
    ...
    (change_div.style.width = "32px"),
      (change_div.style.height = "32px"),
      (change_div.style.position = "absolute"),
      (change_div.style.right = "5px"),
      (change_div.style.top = "80px"),
      (change_div.style.background = "#000");
  ...
  var is_targeted = function (elem, target) {
    return elem
      ? elem === target || is_targeted(elem.parentNode, target)
      : false;
  };
...
var spawn_coins = function (k) {
  for (var j = 0; j < k; j++) {
    ...
    do {
      ...
      i < 1e3 &&
      map.data.solid_tiles.includes(parseInt(map.get_tile(x, y)))
    );
...
var handle_mage_at_screen_edges = function () {
  ...
  var screen = { x: map_pos.x + map.offset.x, y: map_pos.y + map.offset.y };
...
var handle_mage_collisions = function () {
  ...
  for (var i in effects) {
    if (effects[i] != map.enemy_tile) map.data.tiles[effects[i]][1]();
  }
...
var load_level = function (id) {
  ...
  if (lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
    ...
    toast.makeToast(
      "You need " + lvl.cost + " coins to enter level " + id + "!"
    );
  } else {
